...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your mongo_rest account.<br>
        ...
        &mdash; mongo_rest Team
      `
      return sendMail({ toEmail: email, subject: 'mongo_rest - Password Reset', content })
    })
